...
export class ApiService {
  ...
  async startPartido(localId: number, visitId: number): Promise<number> {
    ...
    const resp = await firstValueFrom(
      this.http.post<StartPartidoResponse>(`${environment.apiPython}/api/admin/partidos/start`, {
  ...
  saveRosterAdmin(partidoId: number, items: RosterItemPost[]): Promise<void> {
    ...
    const url = `${environment.apiPython}/api/admin/partidos/${partidoId}/roster`;
  ...
  /* ===== REST varios ===== */
  getEquipos(): Observable<any[]> { return this.http.get<any[]>(`${environment.apiJava}/api/equipos`); }
  getJugadores(equipoId: number): Observable<any[]> { return this.http.get<any[]>(`${environment.apiJava}/api/jugadores?equipoId=${equipoId}`); }
  ...
  async resetMatch(partidoId: number | null) {
    if (partidoId && partidoId > 0) {
      try { 
        await firstValueFrom(this.http.delete(`${environment.apiPython}/api/admin/partidos/${partidoId}/reset`)); 
      } catch {}
    }
    this.clearPartidoLocalState();
    try { await this.hub?.invoke('PeriodControl', this.period$.value); } catch {}
  ...
  async saveMatch(partidoId: number | null) {
    ...
    try {
      await firstValueFrom(this.http.patch(`${environment.apiPython}/api/admin/partidos/${partidoId}/estado`, { estado: 'finalizado' }));
    } catch {
