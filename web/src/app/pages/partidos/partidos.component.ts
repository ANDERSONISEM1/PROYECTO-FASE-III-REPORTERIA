  ...
  <div class="modal-backdrop" *ngIf="showRosters" (click)="backdrop($event)">
    <div class="modal-card roster">
      ...
      <div class="modal-footer">
        ...
        <span class="spacer"></span>
        <button (click)="closeModals()">Cancelar</button>
        <button class="btn-subtle" (click)="resetRosters()">Reset</button>
...
export class PartidosComponent {
  ...
  saveRosters() {
    ...
    this.service.saveRoster(pid, items).subscribe({
      next: () => { 
        this.rosterPartido = items; 
        
        // Finalizar partido automáticamente después de guardar roster
        this.service.updateEstado(pid, 'finalizado').subscribe({
          next: () => {
            this.closeModals();
            this.reload(); // Recargar lista de partidos para mostrar estado actualizado
          },
          error: (err: any) => {
            this.closeModals();
            alert(err?.error?.error || 'Roster guardado pero error al finalizar partido.');
            this.reload(); // Recargar de todas formas
          }
        });
      },
  ...
  resetRosters() {
    // Limpiar todas las selecciones de titulares
    this.rosterSeleccion = {
      local: {},
      visita: {}
    };
  }
