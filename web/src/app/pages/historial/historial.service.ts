...
export class HistorialService {
  private readonly api = `http://localhost:5082`; // Python API para historial
  private readonly apiEquipos = `http://localhost:5081`; // Java API para equipos

  constructor(private http: HttpClient) {}

  getEquipos(): Observable<Team[]> {
    return this.http.get<any[]>(`${this.apiEquipos}/api/equipos`).pipe(
  ...
  getPartidos(equipoId?: number): Observable<Partido[]> {
    const url = equipoId && equipoId > 0
      ? `${this.api}/api/admin/partidos/historial?equipoId=${equipoId}`
      : `${this.api}/api/admin/partidos/historial`;
